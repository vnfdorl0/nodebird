...
const morgan = require('morgan'); // HTTP 요청 로싱을 위한 미들웨어
const path = require('path'); // 파일 및 디멕토리 경로를 다루기 위한 path 모듈
const session = require('express-session'); // 세션 관리를 위한 미들웨어
const nunjucks = require('nunjucks'); // 템플릿 엔지: nunjucks
...
// nunjucks를 설정하여 'views' 디렉토리를 템플릿 파일이 위치한 곳으러 지정
...
app.use(session({
    ...
    saveUninitialized: false, // 초기화되지 않은 세션을 저장하지 않도록 셜정
    secret: process.env.COOKIE_SECRET, // 쿠키 서명을 윙한 비밀키 입력
    cookie: {
...
app.use((err, req, res, next) => {
    ...
    res.locals.error = process.env.NODE_ENV !== 'production' ? err : {}; // 개발 환경에서만 에러 스텍 노출
